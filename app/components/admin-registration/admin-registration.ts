...
export class AdminRegistrationComponent implements OnInit {
  ...
  loadUserPhoto(user: any): void {
    ...
    if (cached) {
      ...
      this.selectedAdUser = this.selectedAdUser ? { ...this.selectedAdUser, photoUrl: cached } : this.selectedAdUser;
      return;
    }

    this.appService.getADUserPhoto(upn).subscribe(res => {
      const photo = res?.data || res?.photoUrl || res?.photo || (typeof res === 'string' ? res : null);
      if (photo) {
        this.photoCache.set(upn, photo);
        user.photoUrl = photo;
        this.adUsers = [...this.adUsers];
        this.selectedAdUser = this.selectedAdUser ? { ...this.selectedAdUser, photoUrl: photo } : this.selectedAdUser;
        this.cdr.markForCheck();
      }
    });
  }
