...
export class EmailListComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (params['appId']) {
      ...
      // Open test form when action=send is passed (from TP Assist)
      this.showTestForm = true;
      this.isDirectAccess = true;
      // Pre-populate for TP Assist navigation
    ...
    this.route.queryParams.subscribe(queryParams => {
      ...
      // Handle action=send for opening test email form (from TP Assist)
  ...
  async loadApplications(): Promise<void> {
    try {
      ...
      if (role === 'USER') {
        ...
        if (userId) {
          response = await firstValueFrom(this.applicationService.getUserApplications(userId, 1, 1000));
        } else {
          response = await firstValueFrom(this.applicationService.getApplications(1, 1000));
        }
      } else {
        response = await firstValueFrom(this.applicationService.getApplications(1, 1000));
      }
  ...
  /**
   * Pre-populate test email form when navigating from TP Assist
   */
