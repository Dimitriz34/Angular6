...
export class LoginComponent implements OnInit {
  ...
  async ngOnInit() {
    // Handle redirect response after returning from Azure AD login
    try {
      ...
      // Determine if this is a real auth failure or an expected post-logout/stale-state error.
      // MSAL throws specific error codes for non-actionable scenarios (e.g. returning from
      // Azure logout, stale interaction state, etc.). These should NOT show a user-facing error.
      if (this.isActionableAuthError(error)) {
  ...
  /**
   * Returns true only for genuine authentication failures that the user should see.
   * Returns false for MSAL housekeeping errors (post-logout redirect, stale state, etc.).
   */
  private isActionableAuthError(error: any): boolean {
    ...
    return !benignPatterns.some(p => combined.includes(p));
  }
