...
import { APPLICATION_LABELS, APPLICATION_MESSAGE_FORMATTERS, APPLICATION_MESSAGES, EMAIL_MESSAGES, MESSAGE_TITLES } from '../../../shared/constants/messages';
...
export class ApplicationFormComponent implements OnInit {
  ...
  // Fields available for user-selectable encryption (AppSecret, AppOwner, OwnerEmail are always encrypted)
  encryptionOptions = [
    { key: 'CoOwner', label: 'Co-Owner Name' },
    { key: 'CoOwnerEmail', label: 'Co-Owner Email' },
    { key: 'EmailServer', label: 'Email Server' },
    { key: 'Port', label: 'Port Number' },
    { key: 'FromEmailAddress', label: 'From Email' },
    { key: 'Body', label: 'Email Body' },
    { key: 'Subject', label: 'Email Subject' }
  ];
  selectedEncryptionFields: string[] = [];
  ...
  /**
  * Auto-populate owner fields with current logged-in user's information (from TP Data Assist)
   */
  ...
  /**
  * Pre-select App User and TP Internal for TP Data Assist navigation
   */
  ...
  getEncryptedFieldsString(): string {
    // Always include mandatory encrypted fields + user-selected ones
    const mandatory = ['AppSecret', 'AppOwner', 'OwnerEmail'];
    const all = [...mandatory, ...this.selectedEncryptionFields];
    return [...new Set(all)].join(',');
  }

  loadEncryptedFields(encryptedFields: string | null | undefined): void {
    const mandatory = ['AppSecret', 'AppOwner', 'OwnerEmail'];
    if (encryptedFields === null || encryptedFields === undefined) {
      // Backward compat: treat NULL as all fields selected
      this.selectedEncryptionFields = this.encryptionOptions.map(o => o.key);
    } else if (encryptedFields === '') {
      this.selectedEncryptionFields = [];
    } else {
      const fields = encryptedFields.split(',').map(f => f.trim());
      // Only load user-selectable fields (exclude mandatory ones)
      this.selectedEncryptionFields = fields.filter(f => !mandatory.includes(f));
    }
  }

  isAllEncryptionSelected(): boolean {
    return this.selectedEncryptionFields.length === this.encryptionOptions.length;
  }

  toggleSelectAllEncryption(): void {
    if (this.isAllEncryptionSelected()) {
      this.selectedEncryptionFields = [];
    } else {
      this.selectedEncryptionFields = this.encryptionOptions.map(o => o.key);
    }
  ...
  async loadApplication(id: number): Promise<void> {
    try {
      ...
      if (response && response.resultData && response.resultData.length > 0) {
        ...
        this.appData = data;
        this.appData.useTPAssist = (data as any).useTPAssist ?? (data as any).UseTPAssist ?? false;
        ...
        // Load encryption field selections
        this.loadEncryptedFields(data.encryptedFields ?? null);
  ...
  async onSubmit(): Promise<void> {
    // Set encrypted fields selection before submitting
    this.appData.encryptedFields = this.getEncryptedFieldsString();
      ...
      this.appService.addApplication(this.appData).subscribe({
        next: async (response: any) => {
          if (response?.resultData?.[0]?.id && response?.resultData?.[0]?.appSecret) {
            ...
            const isInternalApp = this.isTPInternal() || this.appData.isInternalApp;

            if (isInternalApp) {
              try {
                const baseApiUrl = environment.apiUrl.replace(/\/api\/?$/, '');
                await this.appService.sendGuidanceEmail(
                  appId,
                  this.appData.ownerEmail,
                  '',
                  appSecret,
                  baseApiUrl,
                  this.appData.coOwnerEmail || undefined
                ).toPromise();
            ...
            // For TP Internal, guidance is sent immediately; test email is optional
