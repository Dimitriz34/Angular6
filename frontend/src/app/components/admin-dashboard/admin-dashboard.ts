...
export class AdminDashboardComponent implements OnInit {
  ...
  onchangeApp(appId: any) {
    ...
    // Update both charts on selection change
    setTimeout(() => {
      this.initBarChart();
      this.initBreakdownChart();
    }, 0);
  ...
  private createBarChart(canvas: HTMLCanvasElement) {
    ...
    let barCharData: any[];
    if (this.selectedAppId && this.selectedAppId !== 0) {
      // Show only the selected app
      barCharData = this.emailCount.filter(x => x.appId === this.selectedAppId);
    } else {
      // Show top 10 by total sent email (descending)
      barCharData = this.emailCount
        .filter(x => x.appId !== 0 && x.totalSentEmail > 0)
        .sort((a, b) => b.totalSentEmail - a.totalSentEmail)
        .slice(0, 10);
    }
