...
export class UserDashboardComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      const results: any = await firstValueFrom(forkJoin({
        ...
        top5Utilisation: this.dashboardService.getTop5AppsUtilisation(), // Top 5 apps by last 10 days utilisation
      ...
      // Get top 5 apps by last 10 days utilisation directly from dedicated proc
      this.top10Apps = (results.top5Utilisation?.resultData || []).slice(0, 5);
  ...
  initChart() {
    ...
    // Filter chart data by selected apps, or show top 10 by last 30 days volume
    let barCharData: any[];
    if (this.selectedAppIds && this.selectedAppIds.length > 0) {
      barCharData = this.globalChartData
        .filter(x => this.selectedAppIds.includes(x.appId))
        .sort((a: any, b: any) => (b.lastThirtyDaysEmail || 0) - (a.lastThirtyDaysEmail || 0));
    } else {
      barCharData = this.globalChartData
        .filter((x: any) => x.appId !== 0 && (x.lastThirtyDaysEmail || 0) > 0)
        .sort((a: any, b: any) => (b.lastThirtyDaysEmail || 0) - (a.lastThirtyDaysEmail || 0))
        .slice(0, 10);
    }
    
    const labels = barCharData.map(x => x.appName);
    const emails = barCharData.map(x => x.lastThirtyDaysEmail || 0);
    ...
    if (breakdownCanvas) {
      ...
      if (bctx) {
        ...
        appsToAggregate.forEach(app => {
          ...
          aggTotal += app.totalSentEmail || 0;
        });
        
        const labelsPie = ['Today', 'Last 7 Days', 'Monthly', 'Yearly', 'Total'];
        const dataPie = [aggToday, agg7Days, aggMonthly, aggYearly, aggTotal];
